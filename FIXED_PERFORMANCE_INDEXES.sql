...
-- Fixed SQL syntax errors for PostgreSQL compatibility
...
-- ✅ EXPRESSION INDEXES FOR COMPUTED VALUES (FIXED SYNTAX)
CREATE INDEX IF NOT EXISTS idx_orders_date_only 
ON orders(business_id, DATE(created_at), status);

-- FIXED: Removed the problematic EXTRACT expression index
-- Alternative approach using date_trunc function
CREATE INDEX IF NOT EXISTS idx_orders_hourly_analysis 
ON orders(business_id, DATE_TRUNC('hour', created_at), DATE(created_at));
