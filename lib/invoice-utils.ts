...
/**
 * Fetch business information for invoice generation
 */
async function fetchBusinessInfo(): Promise<BusinessInfo | null> {
  try {
...
/**
 * Generate invoice HTML content with business information
 */
    ...
    <html>
      ...
      <body>
        <div class="header">
          <div style="display: flex; justify-content: space-between; align-items: start;">
            <div class="business-info">
              <h1>${getFormattedBusinessName(businessInfo)}</h1>
              ${
                businessInfo?.business_number
                  ? `<p>Business Number: ${businessInfo.business_number}</p>`
                  : ""
              }
              ${
                formatBusinessAddress(businessInfo?.address)
                  ? `<p>${formatBusinessAddress(businessInfo?.address)}</p>`
                  : ""
              }
              ${
              ...
              <p>Email: ${businessInfo?.email || "Not specified"}</p>
            </div>
          ...
          <div class="section">
            ...
            ${
              order.delivery_location
                ? `<p><strong>Delivery Location:</strong> ${order.delivery_location.name}</p>`
                : ""
            }
            ${
              order.rider_name
                ? `<p><strong>Rider:</strong> ${order.rider_name} - ${
                    order.rider_phone || ""
                  }</p>`
                : ""
            }
        ...
        <div class="summary">
          ...
          <div class="summary-row">
            <span>Service Charge (${order.service_charge_rate || 2.5}%):</span>
          ...
          ${
            ...
            <div class="summary-row">
              ...
              <span>${formatCurrency(
                (order.takeaway_packs || 0) * (order.takeaway_pack_price || 0)
              )}</span>
            </div>
          `
              : ""
          ...
          ${
            (order.custom_charges_total || 0) > 0
              ? `
            <div class="summary-row">
              <span>Additional Charges:</span>
              <span>${formatCurrency(order.custom_charges_total)}</span>
            </div>
          `
              : ""
          }
        ...
        <div class="footer">
          ...
          ${
            businessInfo?.phoneNumber
              ? `<p>For any questions, please contact us at ${businessInfo.phoneNumber}</p>`
              : ""
          }
        </div>
      </body>
...
export async function printInvoice(order: Order) {
  try {
    ...
    const htmlContent = generateInvoiceHTML(order, businessInfo);
...
export function downloadInvoiceAsCSV(order: Order) {
  const csvData = [
    ...
    [
      "Service Charge (2.5%)",
      `₦${(order.service_charge || 0).toLocaleString()}`,
    ],
